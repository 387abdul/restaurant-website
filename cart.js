...
let Total_amount = () => {
  if (basket.length != 0) {
    ...
    label.innerHTML = `
    <hr>
  <div class="checkout_area text-center mt-5">
    <h3 >Total Amount :: <span>$${total_amount}</span></h3>
    <button type="button" class="btn btn-danger removeBtn float-right mt-3" onclick="window.location.href='map.html'">Checkout</button>
  </div>`
  }
  else{
    label.innerHTML = `
